...
// For 'headers' exchange type
command.option('-o --values <string>', 'Headers values for "headers" exchange type. (JSON format ex: \'{"key":"value"}\')')
...
} else if (options.exchangeType === 'headers') {
    if (!options.values) logger.warn('No optionnal values specified for "headers" exchange type. (use -o --values <string>)');
}